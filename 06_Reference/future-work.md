...
| **Lock screen power controls** | ✅ **Done** 2026‑09‑26 as the `session_actions` widget (argv-only, destructive actions arm); not exercised through a real lock. Original note: | Sleep/Reboot/Shutdown/Logout. **Security is non-negotiable**: must not bypass authentication, expose protected data, unlock the session, or interfere with PAM or `ext-session-lock`. Destructive actions need confirmation |
