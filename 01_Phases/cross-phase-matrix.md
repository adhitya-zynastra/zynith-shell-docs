...
| Control Center section transition | 6 (`e9e27b0`) | 6A | *pending commit* | Bespoke six-member interpolation state replaced by the shared `MorphTransition` primitive; interpolation maths and visual behaviour unchanged | Refactor, not a behaviour change. The launcher and lock screen can now consume the same primitive |
| `[shell.animation]` ownership | 3 (Motion plugin) | 6 / 6A | `ccfe125`; ADR‑0015 | Motion model moved into the main config schema (`ccfe125`); ADR‑0015 decides the shell becomes sole generator of `animations.kdl` and the plugin is retired — **decided, not implemented** | Until 6B the plugin and main settings both touch motion; the conflict is latent, documented in `motion-settings.md` |
