...
## T‑12 · A hand edit to `animations.kdl` or `glass.kdl` was reverted

- **Cause:** both are generated by the shell from `[shell.animation]` and `[shell.glass]` (ADR‑0015/0016). The next
  config change that alters them overwrites the file — by design, so the file never disagrees with the settings.
- **Fix:** change the setting instead — Personalization (`Super+Alt+A`), or the table in `rice.toml`.
- **Check what the shell did:** `grep niri-fragment` in the shell's log shows each install, rejection or rollback.

## T‑13 · niri reports `duplicate node 'blur', single node expected`

- **Cause:** a `blur { }` block was added to `config.kdl` (or another include) while `rice/glass.kdl` also defines
  one. niri allows exactly one.
- **Fix:** remove the extra block. Blur strength is `[shell.glass].blur_strength`; the generated `glass.kdl` is the
  only place the node should live.

## T‑14 · `Super+Alt+A` (or a script) opens nothing: `settings-open zynith-corner`

- **Cause:** the section was renamed **Personalization** on 2026‑09‑25 (ADR‑0017). The old id has no alias.
- **Fix:** `noctalia msg settings-open personalization`. The shipped `binds.kdl` already uses it.

## T‑15 · Clicking the clock closes the Control Center it just opened

- **Cause:** before `09990f8`, resting on the clock hover-opened Home and the click that followed toggled it shut,
  because the clock is routed to Home as well.
- **Fix:** update to `09990f8` or later — a click that follows a hover-open now confirms it. If it still happens,
  check the pointer did not leave the time cluster between the hover and the click; leaving it re-arms the toggle.

## T‑16 · Holding a shortcut makes a panel flicker open and shut

- **Cause:** a niri bind that spawns `noctalia msg …` without `repeat=false` re-runs on every key repeat (~50/s).
