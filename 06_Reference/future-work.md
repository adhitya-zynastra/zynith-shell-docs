...
| **Runtime checks not yet run** | Tint following a real palette change (needs a wallpaper or `shell_mode` change), and the fragment writer's full-config rollback (needs a deliberately broken niri config). From the second track set: the carousel at logical 2560 and 1536 on the final code (computed only), media → Media routing (nothing was playing), and a held shortcut after `repeat=false` |
