...
| **Lock screen refinement** | ⚠️ **Preset built, not live** (2026‑09‑26) — the focal composition is in `lockscreen.toml`, but a Phase 2 editor layout in `settings.toml` shadows it; it takes effect after `noctalia msg lockscreen-widgets-reset`, which I have not run ([composition.md](../02_Architecture/lockscreen/composition.md)). Original note: treat it as one composition rather than widgets placed around a wallpaper: hierarchy, negative space, alignment, focal point |
| **Lock screen power controls** | ✅ **Built** 2026‑09‑26 as the `session_actions` widget (argv-only, destructive actions arm); not exercised through a real lock. **Amended 2026‑09‑26: never live** — dropped by the shadowing `widget_order` above; live after the reset. Original note: | Sleep/Reboot/Shutdown/Logout. **Security is non-negotiable**: must not bypass authentication, expose protected data, unlock the session, or interfere with PAM or `ext-session-lock`. Destructive actions need confirmation |
| **Lock screen configurability** | Partly: position, anchor, formats and (new) clock `weight` exist per widget. Original note: widget visibility/position/alignment/typography, clock and date formats, password-field style |
...
| **Hangul text renders blank** | An SSID of five Hangul syllables shows as an empty label in the Control Center (Wi‑Fi tile, Network list), in both styles, although Pango is used and Noto Sans CJK KR is installed. Found 2026‑09‑26; pre-existing; not investigated |
| **Control Center open-idle CPU** | ≈ 2.5 % (Zynith) and ≈ 3.3 % (classic) CPU while open and untouched, against ≈ 0.9 % closed ([`tracks-cc-lock-visual.md`](../03_Performance/benchmarks/tracks-cc-lock-visual.md)). Source not investigated |
| **Display face** | Space Grotesk (display) and Inter (interface) are the intended faces and are not installed; `display_font_family` stays empty. The Control Center section title changes at once during a morph rather than crossfading |
| **Lock editor visibility** | The editor surface is on the Bottom layer, so windows cover it; it relies on an empty workspace (the editor's own toast says so). A layer change would need the lock background drawn in the editor |
