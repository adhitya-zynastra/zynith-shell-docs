...
| **Motion: migrate the niri generator** | **Decided, not implemented** — [ADR‑0015](../05_Decisions/ADRs/ADR-0015-niri-animation-ownership.md): the shell becomes the sole generator and the Motion plugin is retired. Deferred deliberately because shipping the generator *without* retiring the plugin means two programs writing the same file. One coherent Phase 6B change |
