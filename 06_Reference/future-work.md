...
| **Zynith Corner — later groups** | Foundation done ([zynith-corner.md](../02_Architecture/configuration/zynith-corner.md)); bar, launcher, widgets, OSD, Control Center and wallpaper UI groups arrive with their systems |
| **Glass: remaining consumers** | Bar, OSD, notifications, dock, desktop widgets and bar-attached panels still use their own opacity keys. Each moves onto `shell::glass` with its redesign |
| **Zynith Corner deep links** | `settings-open` selects a section, not a group; `Super+Alt+A` relies on Motion being the first group |
| **Runtime checks not yet run** | Tint following a real palette change (needs a wallpaper or `shell_mode` change), and the fragment writer's full-config rollback (needs a deliberately broken niri config) |
